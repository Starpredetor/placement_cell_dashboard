...
import { getResumeForStudent, saveResumeForStudent, deleteResumeForStudent } from './ProfilePage';

type ProfileTab = 'overview' | 'academics' | 'placement' | 'training' | 'documents' | 'compliance';
...
const StudentProfileAdminPage: React.FC = () => {
  ...
  const handleResumeDelete = () => {
    if (!student?.id || !window.confirm('Are you sure you want to delete this student\'s uploaded resume?')) return;
  ...
  if (!canManageStudents) {
    return (
      <article className="card access-denied">
        ...
        <p className="text-secondary">Student profile management is available for authorized roles only.</p>
        <Link className="btn-primary button-link" to="/dashboard">Back to Dashboard</Link>
      </article>
    );
  }
  ...
  return (
    <main className="service-layout">
      ...
      <section className="service-content">
        {studentQuery.isLoading && <article className="card">Loading student profile...</article>}
        {studentQuery.isError && <article className="card form-error">Could not load this student profile.</article>}

        {student && (
          <div style={{ display: 'grid', gap: 'var(--space-lg)' }}>
            
            {/* HERO PANEL */}
            <section className="student-profile-hero" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <Link className="btn-primary button-link" to={`/students/${student.id}/edit`}>
                  Edit Profile
                </Link>
            ...
            <section className="student-stat-grid" aria-label="Student stats">
              {getStudentStats(student).map((stat) => (
                <article className="card kpi-card student-stat-card" key={stat.label}>
                  ...
                  <strong className="kpi-value" style={{ fontSize: '22px' }}>{stat.value}</strong>
            ...
            <section className="card student-profile-workspace">
              <div className="workspace-tabs" role="tablist" aria-label="Student profile sections">
                {[
                ...
                <section className="tab-panel">
                  ...
                  <div className="profile-detail-grid">
                    <span>
                      ...
                      {displayValue(student.academic_history?.tenth_percentage)}% ({displayValue(student.academic_history?.tenth_board)})
                    </span>
                    <span>
                      <strong>12th / Diploma %</strong>
                      {displayValue(student.academic_history?.twelfth_or_diploma_percentage)}% ({displayValue(student.academic_history?.twelfth_or_diploma_type)})
                    </span>
                    <span>
                      <strong>BTech Cumulative CGPI</strong>
                      <strong style={{ color: 'var(--color-primary)', display: 'block', fontSize: '15px' }}>
                    ...
                    <div className="form-group-wide" style={{ border: '1px solid var(--color-border)', borderRadius: '8px', padding: '14px', background: '#F8FAFC' }}>
                      <strong>Courses & Certifications Done</strong>
                      <p style={{ margin: '4px 0 0', color: 'var(--color-slate-600)', fontSize: '13px' }}>
              ...
              {activeTab === 'documents' && (
                <section className="tab-panel">
                  ...
                  <p className="text-secondary">
                    Helper Role: Upload or verify student resumes here. Resumes uploaded will be compiled automatically in the TPO job opportunities roster Excel files.
                  </p>
                  ...
                  <div style={{ display: 'grid', gap: '14px', margin: '14px 0' }}>
                    {uploadedResume ? (
                      ...
                      <div className="empty-module-state" style={{ display: 'grid', justifyItems: 'center', gap: '8px' }}>
                        ...
                        <p className="text-secondary" style={{ fontSize: '13px' }}>Upload a dummy resume PDF for this student.</p>
              ...
              {activeTab === 'placement' && (
                <section className="tab-panel">
                  ...
                  <div className="empty-module-state" style={{ background: '#FAF6F6', borderColor: 'rgba(139, 30, 30, 0.15)' }}>
                    <strong>Job Application Funnel: Qualified</strong>
                    <p className="text-secondary" style={{ fontSize: '13px', margin: '4px 0 12px' }}>
                      Student is currently active in the following placement opportunies.
                    </p>
                    <div style={{ display: 'grid', gap: '8px', textAlign: 'left', maxWidth: '520px', margin: '0 auto' }}>
                      <div style={{ padding: '10px', border: '1px solid var(--color-border)', borderRadius: '6px', background: 'white', display: 'flex', justifyContent: 'space-between' }}>
                        <span><strong>Graduate Engineer Trainee</strong> at StartupX</span>
                        <span className="badge badge-eligible" style={{ fontSize: '10px' }}>HR Round</span>
                      </div>
                      <div style={{ padding: '10px', border: '1px solid var(--color-border)', borderRadius: '6px', background: 'white', display: 'flex', justifyContent: 'space-between' }}>
                        <span><strong>Software Analyst</strong> at BigCorp</span>
                        <span className="badge badge-applied" style={{ fontSize: '10px', background: 'rgba(100, 116, 139, 0.1)', color: 'var(--color-slate-600)' }}>Aptitude qualified</span>
                      </div>
                    </div>
                  </div>
                </section>
                  ...
                  <div className="empty-module-state">
                    ...
                    <div style={{ display: 'grid', gap: '8px', textAlign: 'left', maxWidth: '520px', margin: '0 auto' }}>
                      <div style={{ padding: '10px', border: '1px solid var(--color-border)', borderRadius: '6px', background: 'white', display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <span style={{ color: 'var(--color-accent)', fontWeight: 'bold' }}>Present</span>
                      </div>
                      <div style={{ padding: '10px', border: '1px solid var(--color-border)', borderRadius: '6px', background: 'white', display: 'flex', justifyContent: 'space-between' }}>
                      ...
                      <div style={{ padding: '10px', border: '1px solid var(--color-border)', borderRadius: '6px', background: 'white', display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <span><strong>88 / 100 Marks</strong></span>
                      </div>
                    </div>
                  </div>
                </section>
                ...
                <section className="tab-panel">
                  ...
                  <div className="profile-detail-grid">
                    ...
                    <span>
                      ...
                      {displayValue(student.residential_address)}, {student.residential_city} ({displayValue(student.pin_code)})
                    </span>
                  </div>
                </section>
              )}
