...
const DEFAULT_EVENTS: EventAnnouncement[] = [
  {
    ...
    description:
      'A comprehensive session covering full-stack, DevOps, mobile dev, and data science career paths and preparation strategies.',
    date: 'Jun 3, 2026',
  ...
  },
  {
    ...
    description:
      'Hands-on workshop to critique resumes, evaluate ATS scores, and practice mock coding problems in teams.',
    date: 'Jun 7, 2026',
...
const EventsPage: React.FC = () => {
  ...
  const [selectedAdminEvent, setSelectedAdminEvent] = useState<EventAnnouncement | null>(
    null,
  );
  ...
  // The attendee table below reads this. It was previously referenced without
  // ever being declared, so expanding an event's roster threw a ReferenceError
  // at runtime — invisible under CRA, which never type-checked.
  ...
  const isEventToday = (dateStr: string) => {
    ...
    return (
      term.includes(format1.toLowerCase()) || term.includes(format2) || term === 'today'
    );
  };
      ...
      <div style={{ display: 'grid', gap: 'var(--space-md)' }}>
        ...
        >
          ...
          >
            <div>
              ...
              >
                ...
                <span
                  className={`badge ${isOnline ? 'badge-extended' : 'badge-applied'}`}
                  style={{ fontSize: '11px' }}
                >
              ...
              <p className="text-secondary" style={{ margin: 0, fontSize: '13.5px' }}>
                ...
                {event.topic || 'Industry Readiness Guidance'}
              </p>
            </div>
          ...
          <div>
            ...
            <p
              style={{ margin: 0, color: 'var(--color-text-secondary)', lineHeight: 1.6 }}
            >
              {event.description}
            </p>
          </div>
              ...
              <div style={{ display: 'grid', gap: '10px', fontSize: '13px' }}>
                ...
                >
                  <span className="text-secondary">
                    {isOnline ? 'Webinar Join Link' : 'Seminar Classroom/Hall'}
                  </span>
                  {isOnline ? (
                    <a
            ...
            >
              ...
              <div style={{ display: 'grid', gap: '8px', fontSize: '13px' }}>
                ...
                >
                  ...
                  <strong style={{ color: 'var(--color-slate-900)' }}>
                    {event.attire || 'Formal Dress Attire'}
                  </strong>
                </div>
                {event.deadline && (
                  <div
              ...
              >
                ...
                {event.notes ||
                  'Students must report to the venue 15 minutes prior to the given time.'}
              </div>
            </div>
          </div>
            ...
            >
              <div>
                <h2 style={{ fontSize: '18px', margin: 0 }}>
                  Enrolled Attendees Attendance Sheet
                </h2>
                <p className="text-secondary" style={{ margin: 0, fontSize: '13px' }}>
                  Confirm physical or online turnouts for the registered cohort.
                </p>
              </div>
            </div>
            ...
            >
              <table className="student-directory-table">
                ...
                <tbody>
                  ...
                  {Object.keys(enrollments).filter((key) =>
                    key.startsWith(`${event.id}-`),
                  ).length === 0 && (
                    <tr>
                      <td colSpan={3} style={{ textAlign: 'center', padding: '18px' }}>
                        There are currently no enrolled student applications for this
                        seminar.
                      </td>
                    </tr>
                  )}
      ...
      <div style={{ display: 'grid', gap: 'var(--space-lg)' }}>
        <section className="page-header">
          <div>
            ...
            <p className="text-secondary">
              Enroll in guest lectures, industry seminars, and mock camps scheduled by the
              TPO cell.
            </p>
          </div>
        </section>
        ...
        >
          {events.map((event) => {
            ...
            return (
              <article
                className={`opportunity-card ${pastGlow ? 'is-past-event' : ''}`}
                key={event.id}
              >
                <div className="opportunity-header">
                  <div>
                    ...
                    <h2
                      ...
                      style={{ fontSize: '16px', margin: '4px 0 8px 0' }}
                    >
                      {event.title}
                    </h2>
                  </div>
                  <span
                    className={`badge ${hasEnrolled ? 'badge-active' : pastGlow ? 'badge-alumni' : 'badge-eligible'}`}
                  >
                    {hasEnrolled
                      ? status
                      : pastGlow
    ...
    return (
      <div style={{ display: 'grid', gap: 'var(--space-lg)' }}>
        <section className="page-header" style={{ margin: 0, padding: 0 }}>
          <div>
            ...
            <p className="text-secondary" style={{ margin: 0 }}>
              Announce new guest lectures, bootcamps, and mock review drives, and check
              off turnout checklists.
            </p>
          </div>
          {canManageEvents && (
            <button
        ...
        {selectedAdminEvent ? (
          ...
          >
            {events.map((event) => {
              const enrollCount = Object.keys(enrollments).filter((key) =>
                key.startsWith(`${event.id}-`),
              ).length;
              const todayGlow = isEventToday(event.date);
              const pastGlow = isPastEvent(event.date);
