...
const EventSeminarCreatePage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!title || !desc || !date || !time || !speaker || !topic) {
      alert(
        'Please fill out all mandatory fields (Title, Topic, Speaker, Date, and Time).',
      );
      return;
    }
    ...
    if (sessionType === 'OFFLINE' && !venue) {
      alert(
        'Please specify a physical Classroom/Auditorium Venue for this Offline session.',
      );
      return;
    }
    ...
    try {
      ...
      const eventsList: EventAnnouncement[] = storedEvents
  ...
  return (
    <div style={{ display: 'grid', gap: 'var(--space-lg)', justifyItems: 'center' }}>
      <section className="page-header" style={{ width: '100%', maxWidth: '750px' }}>
        <div style={{ width: '100%' }}>
          ...
          <p className="text-secondary" style={{ textAlign: 'left' }}>
            Schedule guest lectures, webinars, and technical career connect briefings.
          </p>
        </div>
      </section>
      ...
      <article
        className="card"
        style={{ width: '100%', maxWidth: '750px', padding: 'var(--space-lg)' }}
      >
