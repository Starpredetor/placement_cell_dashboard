...
const PlacementDriveFastMarkPage: React.FC = () => {
  ...
  const [sessionTurnout, setSessionTurnout] = useState<'Present' | 'Absent' | 'Late'>(
    'Present',
  );
  ...
  const studentsQuery = useQuery(['students-list-drive-fast-mark'], async () => {
  ...
  const handleMarkAttendance = (e: React.FormEvent) => {
    ...
    if (!resolvedStudent) {
      alert(
        `Student with Roll Number "${rollNumber}" is not imported into this drive's applicant roster.`,
      );
      return;
    }
    ...
    try {
      ...
      setConfirmationMessage(
        `updated attendance for ${resolvedStudent.college_roll_no} (${resolvedStudent.full_name}) as ${sessionTurnout.toUpperCase()}`,
      );
      setRollNumber('');
      setSearchQuery('');
    } catch (err) {
  ...
  return (
    <div style={{ display: 'grid', gap: 'var(--space-lg)', justifyItems: 'center' }}>
      ...
      <section className="page-header" style={{ width: '100%', maxWidth: '600px' }}>
        <div style={{ width: '100%' }}>
          ...
          <p className="text-secondary" style={{ textAlign: 'left' }}>
            Log rapid daily placement turnout logs by roll number.
          </p>
        </div>
      </section>
      ...
      >
        <div>
          <span className="service-eyebrow">Drive: {currentDrive?.opportunityName}</span>
          <h2 style={{ fontSize: '20px', marginTop: '4px' }}>
            {currentDrive?.name || 'Loading Drive...'}
          </h2>
          <p className="text-secondary" style={{ fontSize: '13px', margin: 0 }}>
            Scheduled Date: {currentDrive?.date} | Candidates Imported:{' '}
            {eligibleCohort.length}
          </p>
        </div>
