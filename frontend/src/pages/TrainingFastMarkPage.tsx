...
const TrainingFastMarkPage: React.FC = () => {
  ...
  const [sessionTurnout, setSessionTurnout] = useState<'BOTH' | 'MS' | 'AS' | 'ABSENT'>('BOTH');
  ...
  const handleMarkAttendance = async (e: React.FormEvent) => {
    ...
    if (!resolvedStudent) {
      alert(`Student with Roll Number "${rollNumber}" is either not in the 3rd year cohort of ${targetBatch || 'this batch'} or doesn't exist.`);
      return;
    }
    ...
    <div style={{ display: 'grid', gap: 'var(--space-lg)', justifyItems: 'center' }}>
      ...
      <section className="page-header" style={{ width: '100%', maxWidth: '600px' }}>
        <div style={{ width: '100%' }}>
          ...
          <p className="text-secondary" style={{ textAlign: 'left' }}>Rapid-fire student turnout logger.</p>
        </div>
      </section>
