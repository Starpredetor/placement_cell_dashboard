...
const PlacementOpportunityCreatePage: React.FC = () => {
  ...
  const handlePdfIngestion = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        setIngestProgress('Extracting core roles, compensation, and eligibility rules...');
        setTimeout(() => {
          ...
          let parsedSkills = '• Strong foundation in Python/Java & Web Tech\n• Experience with relational databases (SQL)\n• Basic git workflow knowledge';
          let parsedResponsibilities = '• Assist in developing high scalability cloud microservices.\n• Participate in continuous testing & deploy pipelines.\n• Document code logic and api specifications.';
          ...
          setBatch('2026');
          setCompanyIntro(`${parsedCompany} is a leading global technology provider pioneering innovation in enterprise infrastructure and digital solutions.`);
          setSelectionProcess('1. Resume screening & ATS evaluation\n2. Online Technical MCQ Assessment\n3. System Design & Coding Interview Rounds\n4. HR Leadership Round');
          ...
          setIngesting(false);
          setIngestProgress('');
          alert(`Successfully ingested "${file.name}"! Form fields have been auto-populated with extracted parameters.`);
        }, 800);
      }, 800);
    }, 600);
  };
  ...
  const handleAddRole = () => {
    setMultipleRoles([...multipleRoles, { title: '', ctc: '', stipend: '', branches: [], responsibilities: '', skills: '' }]);
  };
  ...
  const handleEnterPreview = (e: React.FormEvent) => {
    ...
    if (!company || !location || !deadline) {
      alert('Please fill out all mandatory fields (Company, Location, and Registration Deadline).');
      return;
    }
  ...
  const handleSave = () => {
    ...
    try {
      ...
      const newJob: JobOpportunity = {
        ...
        location,
        minCgpi: (minCgpi === 'Open' || !minCgpi || isNaN(Number(minCgpi))) ? 0.0 : Number(minCgpi),
        ...
        responsibilities: hasMultipleRoles ? multipleRoles[0].responsibilities : responsibilities,
    ...
    <form onSubmit={handleEnterPreview} style={{ display: 'grid', gap: '24px' }}>
      ...
      <div style={{ 
        ...
        <div style={{ flex: 1, minWidth: '280px', textAlign: 'left' }}>
          <h4 style={{ margin: 0, color: 'var(--color-accent)', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '15px', fontWeight: 700 }}>
            ✨ AI Job Description Ingestor
          </h4>
          <p className="text-secondary" style={{ margin: '4px 0 0 0', fontSize: '12.5px', lineHeight: 1.4, textAlign: 'left' }}>
            Have a placement JD? Upload the PDF to automatically extract roles, compensation packages, branch rules, and eligibility cutoffs!
          </p>
        </div>
      ...
      {ingesting && (
        <div style={{
          ...
          <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--color-slate-700)' }}>
            {ingestProgress}
          </span>
        </div>
      )}

      <h3 style={{ borderBottom: '2px solid var(--color-slate-100)', paddingBottom: '8px', color: 'var(--color-primary)' }}>1. Corporate & Drive Identity</h3>
      ...
      <h3 style={{ borderBottom: '2px solid var(--color-slate-100)', paddingBottom: '8px', color: 'var(--color-primary)', marginTop: '8px' }}>2. Academic Eligibility & Filters</h3>
      <div className="student-form-grid">
        <label className="form-group">
      ...
      <div className="form-group">
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', marginTop: '6px' }}>
          {branchOptions.map(b => (
            <label key={b} style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', fontWeight: 500 }}>
              <input
                type="checkbox"
      ...
      <h3 style={{ borderBottom: '2px solid var(--color-slate-100)', paddingBottom: '8px', color: 'var(--color-primary)', marginTop: '8px' }}>3. Compensation & Service Agreement</h3>
      <div style={{ display: 'flex', gap: '20px', alignItems: 'center', marginBottom: '8px' }}>
        <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontWeight: 600, fontSize: '14.5px', cursor: 'pointer' }}>
          <input
            type="checkbox"
            checked={hasBond}
      ...
      {hasBond && (
        <div className="student-form-grid" style={{ background: 'rgba(139, 30, 30, 0.03)', padding: '16px', borderRadius: '10px', border: '1px dashed rgba(139, 30, 30, 0.15)' }}>
          <label className="form-group">
      ...
      <div style={{ display: 'flex', gap: '20px', alignItems: 'center', marginTop: '4px' }}>
        <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontWeight: 600, fontSize: '14.5px', cursor: 'pointer', color: 'var(--color-accent)' }}>
          <input
            type="checkbox"
      ...
      {hasMultipleRoles && (
        <div style={{ display: 'grid', gap: '20px', background: 'rgba(13, 148, 136, 0.02)', padding: '20px', borderRadius: '12px', border: '1px solid rgba(13, 148, 136, 0.1)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h4 style={{ margin: 0, color: 'var(--color-accent)' }}>Job Designation Profiles</h4>
            <button type="button" className="btn-secondary" style={{ padding: '6px 12px', fontSize: '12px' }} onClick={handleAddRole}>
              + Add Another Role
            </button>
          </div>

          {multipleRoles.map((item, idx) => (
            <div key={idx} style={{ background: '#ffffff', padding: '16px', borderRadius: '8px', border: '1px solid var(--color-border)', display: 'grid', gap: '14px', position: 'relative' }}>
              {multipleRoles.length > 1 && (
                <button 
                  type="button" 
                  style={{ position: 'absolute', top: '10px', right: '10px', border: 'none', background: 'none', color: 'var(--color-error)', fontWeight: 'bold', cursor: 'pointer' }}
                  onClick={() => handleRemoveRole(idx)}
                >
                  ✕ Remove Role
                </button>
              )}
              <h5 style={{ margin: '0 0 4px 0', fontSize: '14px', fontWeight: 'bold', color: 'var(--color-slate-800)' }}>Role #{idx + 1} Profile</h5>
              ...
              <div className="form-group">
                <label style={{ fontSize: '12px', fontWeight: 600 }}>Branch Eligibility for Role #{idx + 1}</label>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', marginTop: '4px' }}>
                  {branchOptions.map(b => (
                    <label key={b} style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '12px' }}>
                      <input
                        type="checkbox"
      ...
      <h3 style={{ borderBottom: '2px solid var(--color-slate-100)', paddingBottom: '8px', color: 'var(--color-primary)', marginTop: '8px' }}>4. Selection & Drive Logistics</h3>
      <div className="student-form-grid">
        <label className="form-group">
      ...
      <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', borderTop: '1px solid var(--color-border)', paddingTop: '16px', marginTop: '8px' }}>
        ...
        <button type="submit" className="btn-success" style={{ background: 'var(--color-primary)' }}>
  ...
  const renderPreview = () => {
    ...
    const branchesToDisplay = currentRole ? currentRole.branches : selectedBranches.length > 0 ? selectedBranches : branchOptions;
    ...
    return (
      <div style={{ display: 'grid', gap: '24px' }}>
        <div style={{ padding: '12px 16px', background: 'rgba(13, 148, 136, 0.05)', borderRadius: '8px', borderLeft: '4px solid var(--color-accent)', fontWeight: 500, fontSize: '13.5px', color: 'var(--color-accent)' }}>
          🔍 <strong>Drive Preview Mode:</strong> This matches exactly how the placement drive details sheet will load for matching candidates. Review all eligibility parameters prior to publishing.
        </div>

        <article className="card" style={{ display: 'grid', gap: '24px', border: '1px solid var(--color-accent)', boxShadow: '0 0 15px rgba(13, 148, 136, 0.1)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px', borderBottom: '1px solid var(--color-border)', paddingBottom: '20px' }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                <span className="opportunity-company" style={{ fontSize: '16px', fontWeight: 600, color: 'var(--color-primary)' }}>{company || 'Company Name'}</span>
                <span className="badge badge-alumni" style={{ fontSize: '11px', background: 'var(--color-slate-100)', color: 'var(--color-slate-700)' }}>Batch {batch}</span>
                <span className="badge badge-today-pulse" style={{ fontSize: '10px' }}>PREVIEW</span>
              </div>
              <h1 style={{ fontSize: '24px', margin: '6px 0 4px 0', color: 'var(--color-slate-900)' }}>
                {hasMultipleRoles ? `Multiple Roles (${multipleRoles.length})` : role || 'Job Title'}
              </h1>
              <p className="text-secondary" style={{ margin: 0, display: 'flex', gap: '12px', flexWrap: 'wrap', fontSize: '13px' }}>
                <span>📍 {location || 'Job Location'}</span>
                {companyLink && (
                  <span>
                    🔗 <a href={companyLink} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-accent)', fontWeight: 600 }}>Visit Website</a>
                  </span>
                )}
              </p>
            </div>
            
            <button type="button" className="btn-primary" style={{ background: 'var(--color-primary)' }} onClick={() => setShowConfirmModal(true)}>
          ...
          {companyIntro && (
            <div>
              ...
              <p style={{ margin: 0, color: 'var(--color-text-secondary)', lineHeight: 1.6 }}>{companyIntro}</p>
            </div>
          )}
            ...
            <div className="card" style={{ border: '1px solid var(--color-border)', padding: '16px', background: 'var(--color-bg-main)' }}>
              ...
              <div style={{ display: 'grid', gap: '10px', fontSize: '13px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid var(--color-border)', paddingBottom: '6px' }}>
                  ...
                  <strong style={{ color: 'var(--color-slate-900)' }}>{minCgpi} CGPI</strong>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid var(--color-border)', paddingBottom: '6px' }}>
                  <span className="text-secondary">Maximum Live KTs / Backlogs</span>
                  <strong style={{ color: 'var(--color-slate-900)' }}>{maxKts} KTs</strong>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid var(--color-border)', paddingBottom: '6px' }}>
                  <span className="text-secondary">10th Marks Criterion</span>
                  <strong style={{ color: 'var(--color-slate-900)' }}>{tenthPercent}</strong>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid var(--color-border)', paddingBottom: '6px' }}>
                  <span className="text-secondary">12th/Diploma Criterion</span>
                  <strong style={{ color: 'var(--color-slate-900)' }}>{twelfthPercent}</strong>
                </div>
            ...
            <div className="card" style={{ border: '1px solid var(--color-border)', padding: '16px', background: 'var(--color-bg-main)', display: 'grid', alignContent: 'start', gap: '12px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                <div style={{ background: '#ffffff', padding: '10px', borderRadius: '6px', border: '1px solid var(--color-border)' }}>
                  ...
                  <strong style={{ fontSize: '15px', color: 'var(--color-slate-900)' }}>{ctcToDisplay || '10 LPA'}</strong>
                </div>
                <div style={{ background: '#ffffff', padding: '10px', borderRadius: '6px', border: '1px solid var(--color-border)' }}>
              ...
              {hasBond ? (
                <div style={{ background: 'rgba(217, 119, 6, 0.05)', padding: '10px 14px', borderRadius: '6px', borderLeft: '4px solid var(--color-warning)', fontSize: '12.5px' }}>
                  ...
                  Selected candidates must sign a service bond of <strong>{bondDuration || '2 years'}</strong> with a breach penalty of <strong>{bondPenalty || 'Rs. 2,00,000'}</strong>.
                </div>
              ) : (
          ...
          {hasMultipleRoles && multipleRoles.length > 0 && (
            <div style={{ display: 'grid', gap: '10px', borderTop: '1px solid var(--color-border)', paddingTop: '16px' }}>
              <span className="service-eyebrow" style={{ color: 'var(--color-accent)' }}>Multi-role Recruitment Profiles (Interactive Preview)</span>
                ...
                {multipleRoles.map((roleOpt, idx) => (
                  <button
                    ...
                    style={{
                      ...
                      color: previewRoleTab === idx ? '#ffffff' : 'var(--color-slate-600)',
                      borderColor: previewRoleTab === idx ? 'var(--color-accent)' : 'var(--color-border)',
                      padding: '6px 12px',
                      fontSize: '12.5px',
                      borderRadius: '20px'
                    }}
          ...
          <div style={{ borderTop: '1px solid var(--color-border)', paddingTop: '16px', display: 'flex', flexWrap: 'wrap', gap: '24px', background: 'rgba(15, 23, 42, 0.02)', padding: '16px', borderRadius: '8px' }}>
            <div style={{ minWidth: '150px' }}>
              ...
              <strong>
                {deadline ? new Date(deadline).toLocaleString('en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : 'No deadline selected'}
              </strong>
            </div>
            ...
            {registrationLink && (
              <div style={{ minWidth: '150px' }}>
                ...
                <span style={{ color: 'var(--color-primary)', fontWeight: 'bold', fontSize: '13px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', borderTop: '1px solid var(--color-border)', paddingTop: '16px' }}>
          <button type="button" className="btn-secondary" onClick={() => setIsPreviewMode(false)}>
  ...
  return (
    <div style={{ display: 'grid', gap: 'var(--space-lg)', justifyItems: 'center' }}>
      <section className="page-header" style={{ width: '100%', maxWidth: '800px' }}>
        <div style={{ width: '100%' }}>
          ...
          <h1 style={{ textAlign: 'left' }}>
            {isPreviewMode ? 'Preview Drive: ' + (company || 'Corporate Opening') : 'Create Placement Drive Opening'}
          </h1>
      ...
      <article className="card" style={{ width: '100%', maxWidth: '800px', padding: 'var(--space-lg)' }}>
        ...
        <div style={{
          ...
          <div className="card animate-scaleUp" style={{
            ...
            <p className="text-secondary" style={{ margin: 0, fontSize: '14px', lineHeight: 1.5 }}>
              Are you sure you want to publish the placement drive for <strong>{company}</strong>? 
              This will instantly deploy the opening to the Placements Board, making it searchable for matching eligible candidates in Batch <strong>{batch}</strong>.
            </p>
            <div style={{
              display: 'flex',
              gap: '10px',
