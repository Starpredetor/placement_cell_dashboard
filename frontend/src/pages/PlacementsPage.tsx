...
const DEFAULT_JOBS: JobOpportunity[] = [
  {
    ...
    batch: '2026',
    companyIntro:
      'StartupX is an agile tech pioneer specializing in SaaS development and automation tooling for next-gen companies.',
  ...
  },
  {
    ...
    batch: '2026',
    companyIntro:
      'BigCorp Solutions is a multinational engineering consulting firm operating massive enterprise tech infrastructures worldwide.',
...
const PlacementsPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'opportunities' | 'attendance'>(
    'opportunities',
  );
  ...
  const getEligibility = (student: StudentProfile | undefined, job: JobOpportunity) => {
    ...
    if (!hasBranch) {
      return {
        eligible: false,
        reason: `Branch restricted. Eligible branches: ${branches.join(', ')}`,
      };
    }
    ...
    if (cgpi < job.minCgpi) {
      return {
        eligible: false,
        reason: `CGPI threshold not met. Required: ${job.minCgpi} (Your CGPI: ${cgpi})`,
      };
    }
    ...
    if (liveKts > job.maxKts) {
      return {
        eligible: false,
        reason: `KT limits exceeded. Allowed: ${job.maxKts} (Your KTs: ${liveKts})`,
      };
    }
    ...
    if (job.tenthPercent && job.tenthPercent !== 'Open') {
      ...
      if (studentTenth < requiredTenth) {
        return {
          eligible: false,
          reason: `10th percentage threshold not met. Required: ${job.tenthPercent} (Your 10th: ${studentTenth}%)`,
        };
      }
    }
    ...
    if (job.twelfthPercent && job.twelfthPercent !== 'Open') {
      const studentTwelfth = Number(
        student.academic_history?.twelfth_or_diploma_percentage || 0,
      );
      const requiredTwelfth = Number(job.twelfthPercent.replace(/%/g, ''));
      if (studentTwelfth < requiredTwelfth) {
        return {
          eligible: false,
          reason: `12th/Diploma percentage threshold not met. Required: ${job.twelfthPercent} (Your 12th/Diploma: ${studentTwelfth}%)`,
        };
      }
    }
  ...
  const handleApply = (jobId: number) => {
    ...
    alert(
      `Successfully applied to ${jobs.find((j) => j.id === jobId)?.company}! Your resume is linked.`,
    );
  };
  ...
  const handleExportHR = (job: JobOpportunity) => {
    ...
    link.setAttribute(
      'download',
      `HR_Roster_${job.company.replace(/\s+/g, '_')}_Applicants.csv`,
    );
  ...
  const handleMarkDriveAttendance = (
    driveId: number,
    studentId: number,
  ...
  const renderJobDetailView = (job: JobOpportunity) => {
    ...
    const currentRole =
      job.hasMultipleRoles && job.multipleRoles && job.multipleRoles.length > 0
        ? job.multipleRoles[activeRoleTab]
        : null;
    ...
    const respToDisplay = currentRole
    ...
    return (
      <div style={{ display: 'grid', gap: 'var(--space-md)' }}>
        ...
        <article
          className="card"
          style={{ display: 'grid', gap: '24px', padding: 'var(--space-lg)' }}
        >
          {/* Header Banner */}
          <div
            style={{
          ...
          >
            <div>
              ...
              >
                {job.hasMultipleRoles
                  ? `Multiple Roles (${job.multipleRoles.length})`
                  : job.role}
              </h1>
              <p
              ...
              <div style={{ display: 'grid', gap: '10px', fontSize: '13px' }}>
                ...
                >
                  ...
                  <strong style={{ color: 'var(--color-slate-900)' }}>
                    {job.minCgpi === 0 ? 'Open' : `${job.minCgpi} CGPI`}
                  </strong>
                </div>
                <div
                  style={{
                  ...
                  <div
                    style={{
                      ...
                      background: eligibility.eligible
                        ...
                        : 'rgba(239, 68, 68, 0.05)',
                      borderLeft: `4px solid ${eligibility.eligible ? 'var(--color-success)' : 'var(--color-error)'}`,
                    }}
                  >
                    <strong
                      style={{
            ...
            >
              ...
              {job.hasBond ? (
                ...
                >
                  ...
                  <strong>{job.bondDuration || '2 years'}</strong> with a breach penalty
                  of <strong>{job.bondPenalty || 'Rs. 2,00,000'}</strong>.
                </div>
              ) : (
                <div
                  style={{
          ...
          {job.hasMultipleRoles && job.multipleRoles && job.multipleRoles.length > 0 && (
            ...
            >
              <span className="service-eyebrow" style={{ color: 'var(--color-accent)' }}>
                Multi-role Recruitment Profiles
              </span>
          ...
          >
            <div>
              ...
              >
                {respToDisplay ||
                  'No detailed responsibilities specified. Refer to the attached JD.'}
              </div>
            </div>
            <div>
              <h3
          ...
          >
            <div style={{ minWidth: '150px' }}>
              ...
              <strong
                style={{
                  color: isPastDeadline ? 'var(--color-error)' : 'var(--color-slate-900)',
                }}
              >
                ...
                {isPastDeadline
                  ? ' (Expired)'
                  : ` (${isTodayDeadline ? 'Today' : 'Upcoming'})`}
              </strong>
            </div>
        ...
        {!isStudent && (
          <article
            className="card"
            style={{ display: 'grid', gap: '14px', marginTop: '10px' }}
          >
            <div
              style={{
                display: 'flex',
            ...
            >
              <div>
                <h2 style={{ fontSize: '18px', margin: 0 }}>
                  Funnel Candidate Progression
                </h2>
                <p className="text-secondary" style={{ margin: 0, fontSize: '13px' }}>
                  Promote qualifying candidates across interview slots or review their
                  resumes.
                </p>
              </div>
            </div>
              ...
              <table className="student-directory-table">
                ...
                <tbody>
                  {Object.keys(applications)
                    ...
                    .map((key) => {
                      const app = applications[key];
                      const studentDetail = studentsList.find(
                        ...
                        <tr key={key}>
                          <td>
                            ...
                            <span className="table-subtext">
                              {studentDetail?.branch || 'Branch loading...'}
                            </span>
                          </td>
                          <td>
                            <span>
                              {studentDetail?.academic_history?.se_cgpi || '8.2'} CGPI
                            </span>
                            <span className="table-subtext">
                              {studentDetail?.academic_history?.live_kt ?? 0} backlogs
                            </span>
                          </td>
                          <td>
                            <select
                  ...
                  {Object.keys(applications).filter((key) => key.startsWith(`${job.id}-`))
                    .length === 0 && (
                    <tr>
                      <td colSpan={5} style={{ textAlign: 'center', padding: '18px' }}>
                        There are currently no registered student applications for this
                        drive.
                      </td>
                    </tr>
                  )}
      ...
      return (
        ...
        >
          {jobs.map((job) => {
            ...
            return (
              <article
                className={`opportunity-card ${deadlinePast ? 'is-past-event' : ''}`}
                key={job.id}
              >
                <div className="opportunity-header">
                  <div>
                    ...
                    <h2 className="opportunity-title">
                      {job.hasMultipleRoles
                        ? `Multiple Roles (${job.multipleRoles?.length || 2})`
                        : job.role}
                    </h2>
                  </div>
                  <span
                    className={`badge ${hasApplied ? 'badge-applied' : isEligible ? 'badge-eligible' : 'badge-not-eligible'}`}
                  >
                ...
                <div
                  className={`opportunity-eligibility ${isEligible ? 'eligible' : 'ineligible'}`}
                >
                  {isEligible
                    ? 'Eligible: You meet all eligibility criteria'
    ...
    return (
      <div style={{ display: 'grid', gap: 'var(--space-lg)' }}>
        <section className="page-header" style={{ margin: 0, padding: 0 }}>
          <div>
            ...
            <p className="text-secondary" style={{ fontSize: '13px', margin: 0 }}>
              Monitor qualifying funnel phases, select a job card to manage candidates,
              and export complete rosters to HR.
            </p>
          </div>
          {canManageOpenings && (
            <button
        ...
        {selectedAdminJob ? (
          ...
          >
            {jobs.map((job) => {
              const applicantCount = Object.keys(applications).filter((key) =>
                key.startsWith(`${job.id}-`),
              ).length;
              const deadlinePast = isPast(job.deadline);
              return (
                <article
                  className={`opportunity-card animate-scaleUp ${deadlinePast ? 'is-past-event' : ''}`}
                  key={job.id}
                >
                  <div className="opportunity-header">
                    <div>
      ...
      return (
        <article className="card">
          ...
          <p className="text-secondary" style={{ marginBottom: '16px' }}>
            View scheduled daily placement rounds attendance and verification states.
          </p>
              ...
              <tbody>
                {myDrives.map((d) => {
                  ...
                  return (
                    <tr key={d.id}>
                      ...
                      <td>
                        <span
                          className={`badge ${status === 'Present' ? 'badge-active' : status === 'Late' ? 'badge-extended' : 'badge-not-eligible'}`}
                        >
                          {status}
                        </span>
                      </td>
                ...
                {myDrives.length === 0 && (
                  <tr>
                    ...
                    >
                      No placement drive attendance logs found. (You will appear here when
                      you apply to opportunities and drives are scheduled).
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      <div style={{ display: 'grid', gap: 'var(--space-lg)' }}>
        <section className="page-header" style={{ margin: 0, padding: 0 }}>
          <div>
            ...
            <p className="text-secondary" style={{ fontSize: '13px', margin: 0 }}>
              Coordinate daily physical round checklists. Click a drive card to verify
              imported candidates.
            </p>
          </div>
          {canManageOpenings && (
        ...
        {selectedDrive ? (
            ...
            <article className="card" style={{ display: 'grid', gap: '14px' }}>
              ...
              >
                <div>
                  ...
                  <p className="text-secondary" style={{ margin: 0, fontSize: '13px' }}>
                    Company: {selectedDrive.opportunityName} | Date: {selectedDrive.date}{' '}
                    | Total Imported Candidates:{' '}
                    {Object.keys(selectedDrive.attendance).length}
                  </p>
                </div>
                ...
                <button
                  ...
                  onClick={() =>
                    navigate(`/placements/drives/mark-single/${selectedDrive.id}`)
                  }
                >
                  ...
                  <tbody>
                    {Object.keys(selectedDrive.attendance).map((studentIdStr) => {
                      ...
                      return (
                        <tr key={targetId}>
                          <td>
                            <strong>
                              {studentDetail?.full_name || 'Loading Student...'}
                            </strong>
                          </td>
                          <td>
                            <code>{studentDetail?.college_roll_no || 'N/A'}</code>
                          </td>
                    ...
                    {Object.keys(selectedDrive.attendance).length === 0 && (
                      <tr>
                        ...
                        >
                          No candidates imported. (This happens when no student has
                          applied to the opportunity prior to scheduling the drive).
                        </td>
                      </tr>
                    )}
                  </tbody>
          ...
          >
            {drives.map((d) => {
              ...
              return (
                <article
                  className={`opportunity-card animate-scaleUp ${pastGlow ? 'is-past-event' : ''}`}
                  key={d.id}
                >
                  <div className="opportunity-header">
                    <div>
    ...
    <div style={{ display: 'grid', gap: 'var(--space-md)' }}>
      ...
      <section className="page-header" style={{ marginBottom: 'var(--space-sm)' }}>
        <div>
          ...
          <p className="text-secondary">
            Explore corporate opportunities and log drive-specific student rosters
            attendance.
          </p>
        </div>
      </section>
      ...
      <section className="premium-tab-deck">
        ...
        >
          ...
          <span className="premium-tab-subtitle">
            Manage open positions, cutoffs, qualifications, and HR rosters
          </span>
        </button>
        <button
          type="button"
        ...
        >
          ...
          <span className="premium-tab-subtitle">
            Track physical daily turnout checklists for active corporate rounds
          </span>
        </button>
      </section>
