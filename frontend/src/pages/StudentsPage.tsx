...
const StudentsPage: React.FC = () => {
  ...
  const canManageStudents = user?.role === 'SUPER_ADMIN' || user?.role === 'TPO' || user?.role === 'HOD';
  ...
  const filteredResults = React.useMemo(() => {
    ...
    return studentsQuery.data.results.filter((s) => {
      const matchBranch = !branchFilter || s.branch.toLowerCase().includes(branchFilter.toLowerCase());
  ...
  return (
    <div style={{ display: 'grid', gap: 'var(--space-lg)' }}>
      <section className="page-header">
        ...
        {canManageStudents && (
          <div style={{ display: 'flex', gap: '10px' }}>
            <button type="button" className="btn-secondary" onClick={handleAddDemoStudent}>
      ...
      <section className="students-layout">
        {/* Side filter panel */}
        <aside className="card" style={{ display: 'grid', gap: '14px', position: 'sticky', top: '18px' }}>
          ...
          <div className="form-group">
            ...
            <select id="branch" value={branchFilter} onChange={(e) => setBranchFilter(e.target.value)}>
              ...
              <option value="Electronics and Telecommunications">Electronics & Telecommunications (EXTC)</option>
              <option value="Electronics and Computer">Electronics & Computer (ECE)</option>
          ...
          <div className="form-group">
            ...
            <select id="entry" value={entryFilter} onChange={(e) => setEntryFilter(e.target.value)}>
        ...
        <section style={{ display: 'grid', gap: 'var(--space-md)' }}>
          {studentsQuery.isLoading && <article className="card">Loading student profiles...</article>}
          {studentsQuery.isError && <article className="card form-error">Could not fetch students from TPO server.</article>}
                ...
                <table className="student-directory-table">
                  ...
                  <tbody>
                    {filteredResults.map((student) => (
                      <tr key={student.id}>
                        <td>
                          <strong style={{ display: 'block', fontSize: '14px', color: 'var(--color-slate-900)' }}>
                            {student.full_name}
                          </strong>
                        ...
                        <td>
                          <code style={{ fontSize: '12.5px', background: 'var(--color-slate-100)', padding: '2px 6px', borderRadius: '4px' }}>
                            {student.college_roll_no}
                          </code>
                        </td>
                        <td>
                          <span style={{ display: 'block', fontWeight: 500 }}>{student.branch}</span>
                          <span className="table-subtext">{student.batch || 'Batch unset'}</span>
                        </td>
                        <td>
                          <strong style={{ color: 'var(--color-slate-900)' }}>
                            {student.academic_history?.se_cgpi || 'N/A'} CGPI
                          </strong>
                          <span className="table-subtext" style={{ color: student.academic_history?.live_kt ? 'var(--color-error)' : 'var(--color-slate-500)' }}>
                            {student.academic_history?.live_kt ?? 0} Live KTs
                          </span>
                        </td>
                        <td>
                          <div style={{ display: 'flex', gap: '8px' }}>
                            <Link to={`/students/${student.id}`} className="btn-secondary button-link" style={{ padding: '6px 12px', fontSize: '12px' }}>
                              View
                            </Link>
                            
                            {/* Volunteer or Admin can Edit details */}
                            <Link to={`/students/${student.id}/edit`} className="btn-success button-link" style={{ padding: '6px 12px', fontSize: '12px' }}>
                              Edit
                            </Link>
